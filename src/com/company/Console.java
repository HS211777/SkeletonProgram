...
public class Console {
    ...
    public void printLeaderBoard(){
        ...
        for (int i = 0; i < scores.size(); i++) {
            ...
            String temp1[] = scores.get(i).split(" ");
            String tempstr = temp[3].trim();
            double score = Integer.parseInt(tempstr);
            int score1 = (int) score;
            String name = temp1[1];
            score myScore = new score(name, score1);
    ...
    public static void sortArray(ArrayList<score> array){
        boolean valid = false;
        while (!valid){
            for (int i = 0; i < array.size()-1; i++){
                ...
                boolean end = false;
                while (count <= i && !end){
                    if (array.get(count).getScore() > array.get(i+1).getScore()){
                        score temp = array.get(count);
